...
app.put('/api/activities/checkout', async (request, response) => {
    ...
    const db = await openDatabase();/*abre banco de dados*/
    const vehicle = await db.get
    (`    
           SELECT * FROM vehicles WHERE label = ?  
         `, [label]);/*verifico se o veículo existe*/
         ...
         if(vehicle) {
            ...
            (`    
                 ...
                 if(activityOpen){/*se existir pendencias*/
            ...
            db.close();/*se não exixte ele manda mensagem*/
            response.send({
                    message: `Veículo [${label}] não cadastrado`
            });
